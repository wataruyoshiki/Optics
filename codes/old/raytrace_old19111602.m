function [retval, Y, Z] = raytrace (lens1, lens2, gaussdata, y_0, Rho)
  ...
  N = size(r,2);          % レンズ面数
  L = zeros(1,N);         % 面iへの入射光線が光軸と交わる距離
  Ld = zeros(1,N);        % 面iからの屈折光線が光軸と交わる距離
  U = zeros(1,N);         % 面iへの入射光線が光軸となす角
  Ud = zeros(1,N);        % 面iからの屈折光線が光軸となす角
  I = zeros(1,N);         % 面iへの入射角
  Id = zeros(1,N);        % 面iからの屈折角
  Y = zeros(1,N);         % 面iへの入射高さ
  Z = zeros(1,N);         % 面iの入射座標のザグ量
